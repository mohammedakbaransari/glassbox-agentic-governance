...
class TestWriteAheadLogSQLite:
    ...
    def test_partial_side_effects_preserved_on_restart(self):
        ...
        entry = wal1.begin_transaction(rec.decision_id, rec)
        wal1.mark_side_effect(entry.entry_id, "audit_saved", success=True)
        wal1.mark_side_effect(
            entry.entry_id, "workflow_created", success=False, error_msg="timeout"
        )
    ...
    def test_checkpoint_created_at_interval(self):
        ...
        with sqlite3.connect(self._db) as conn:
            count = conn.execute("SELECT COUNT(*) FROM wal_checkpoints").fetchone()[0]
        assert count >= 1
...
class TestTamperEvidentAuditLogger:
    ...
    def test_purge_old_records_removed(self):
        """GB-040: an unconditional DELETE permanently broke hash-chain
        verification, so this method is no longer implemented -- callers must
        migrate to glassbox.app.sealer.SegmentSealer.purge()."""
        logger = self._logger()
        logger.log_action("u1", "a1", "res", "r1", "success")

        with pytest.raises(NotImplementedError):
            logger.purge_old_records(days=1)

        # And, crucially, nothing was deleted as a side effect of the call.
        stats = logger.get_stats()
...
class TestPolicyEngineO5ExceptionHandling:
    ...
    def test_exception_does_not_leak_internal_detail(self):
        from glassbox.governance.models import DecisionContext, DecisionType
...
class TestStageLantencyTracking:
    ...
    def test_stage_registry_latency_populated(self):
        from glassbox.governance.stage_registry import StageExecutionResult, StageRegistry
