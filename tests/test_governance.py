...
from glassbox.governance.policy_engine import DEFAULT_POLICIES, Policy, PolicyEngine
...
def _req(
    ...
    ctx = DecisionContext(
        confidence=conf, environment=env, currency=currency, jurisdiction=jurisdiction
    )
...
class TestNewDecisionTypes(unittest.TestCase):
    ...
    def test_clinical_request_processed(self):
        p = _pipeline()
        r = p.process(
            _req(dtype=DecisionType.CLINICAL, payload={"dose_mg": 10, "drug_name": "amoxicillin"})
        )
    ...
    def test_trading_request_processed(self):
        ...
        r = p.process(
            _req(
                dtype=DecisionType.TRADING,
                payload={"symbol": "AAPL", "quantity": 100, "order_value": 15000},
            )
        )
        self.assertIsNotNone(r.final_status)
...
class TestNewPolicies(unittest.TestCase):
    ...
    def test_fin002_large_transfer_fails(self):
        r = self.pe.evaluate(
            ...
            {"amount": 6_000_000, "destination_account": "ACC-1", "reference": "REF-1"},
            self.ctx,
        )
    ...
    def test_fin002_normal_transfer_passes(self):
        r = self.pe.evaluate(
            DecisionType.FINANCIAL,
            {"amount": 1_000, "destination_account": "ACC-1", "reference": "REF-1"},
            self.ctx,
        )
    ...
    def test_fin003_missing_counterparty_fails(self):
        r = self.pe.evaluate(DecisionType.FINANCIAL, {"amount": 200_000}, self.ctx)
        self.assertFalse(r.passed)
    ...
    def test_fin005_round_below_threshold_warns(self):
        r = self.pe.evaluate(
            DecisionType.FINANCIAL,
            {"amount": 9_800, "destination_account": "ACC-1", "reference": "REF-1"},
            self.ctx,
        )
    ...
    def test_proc004_no_sole_source_flag_passes(self):
        ...
        r = self.pe.evaluate(
            DecisionType.PROCUREMENT,
            {"amount": 50_000, "supplier_id": "SUP-001", "category": "hardware"},
            self.ctx,
        )
        proc004_violations = [v for v in r.violations if "PROC-004" in v]
        self.assertEqual(len(proc004_violations), 0)
    ...
    def test_proc006_debarred_supplier_fails(self):
        r = self.pe.evaluate(
            DecisionType.PROCUREMENT,
            {"amount": 5_000, "supplier_id": "DEBARRED-001", "category": "hardware"},
            self.ctx,
        )
        self.assertFalse(r.passed)
        self.assertTrue(any("PROC-006" in v for v in r.violations))
    ...
    def test_clin001_controlled_no_dea_fails(self):
        r = self.pe.evaluate(
            ...
            {"drug_name": "oxycodone", "drug_class": "schedule_ii", "dose_mg": 5},
            self.ctx,
        )
        self.assertFalse(r.passed)
    ...
    def test_clin001_non_controlled_passes(self):
        r = self.pe.evaluate(
            DecisionType.CLINICAL,
            {"drug_name": "amoxicillin", "drug_class": "antibiotic", "dose_mg": 500},
            self.ctx,
        )
        clin001_violations = [v for v in r.violations if "CLIN-001" in v]
        self.assertEqual(len(clin001_violations), 0)
    ...
    def test_clin002_overdose_fails(self):
        r = self.pe.evaluate(
            DecisionType.CLINICAL,
            {"dose_mg": 1000, "max_dose_mg": 500, "drug_name": "paracetamol"},
            self.ctx,
        )
        self.assertFalse(r.passed)
    ...
    def test_clin002_safe_dose_passes(self):
        r = self.pe.evaluate(
            DecisionType.CLINICAL,
            {"dose_mg": 200, "max_dose_mg": 500, "drug_name": "ibuprofen"},
            self.ctx,
        )
    ...
    def test_clin002_dosage_mg_alias_fails(self):
        r = self.pe.evaluate(
            DecisionType.CLINICAL,
            {"dosage_mg": 1000, "max_dose_mg": 500, "drug_name": "paracetamol"},
            self.ctx,
        )
        self.assertFalse(r.passed)
        self.assertTrue(any("CLIN-002" in v for v in r.violations))
    ...
    def test_trade001_over_limit_fails(self):
        r = self.pe.evaluate(
            ...
            {"symbol": "TSLA", "notional": 20_000_000, "position_limit": 10_000_000},
            self.ctx,
        )
        self.assertFalse(r.passed)
    ...
    def test_trade001_within_limit_passes(self):
        r = self.pe.evaluate(
            DecisionType.TRADING,
            {"symbol": "TSLA", "notional": 5_000_000, "position_limit": 10_000_000},
            self.ctx,
        )
    ...
    def test_trade002_fat_finger_fails(self):
        r = self.pe.evaluate(
            DecisionType.TRADING,
            {"symbol": "AAPL", "quantity": 100_000, "avg_daily_qty": 1_000},
            self.ctx,
        )
        self.assertFalse(r.passed)
    ...
    def test_trade002_normal_size_passes(self):
        r = self.pe.evaluate(
            DecisionType.TRADING,
            {"symbol": "AAPL", "quantity": 500, "avg_daily_qty": 1_000},
            self.ctx,
        )
    ...
    def test_gen001_ssn_in_content_fails(self):
        r = self.pe.evaluate(
            DecisionType.CONTENT, {"content": "Patient SSN is 123-45-6789 per records"}, self.ctx
        )
        self.assertFalse(r.passed)
    ...
    def test_gen001_email_in_content_fails(self):
        r = self.pe.evaluate(
            DecisionType.CONTENT, {"content": "Contact user@example.com for details"}, self.ctx
        )
        self.assertFalse(r.passed)

    def test_gen001_clean_content_passes(self):
        r = self.pe.evaluate(
            DecisionType.CONTENT, {"content": "The quarterly report shows strong growth."}, self.ctx
        )
    ...
    def test_gen002_eu_automated_decision_fails(self):
        ...
        r = self.pe.evaluate(
            ...
            {"affects_individual": True, "content": "Loan application auto-denied"},
            ctx,
        )
        self.assertFalse(r.passed)
    ...
    def test_gen002_non_eu_jurisdiction_passes(self):
        ...
        r = self.pe.evaluate(
            DecisionType.CONTENT, {"affects_individual": True, "content": "Decision made"}, ctx
        )
        gen002_violations = [v for v in r.violations if "GEN-002" in v]
        self.assertEqual(len(gen002_violations), 0)
...
class TestDecisionExplainer(unittest.TestCase):
    ...
    def test_blocked_decision_has_explanation(self):
        resp = self.pipeline.process(_req(payload={"amount": 700_000, "category": "hardware"}))
    ...
    def test_executed_decision_has_explanation(self):
        resp = self.pipeline.process(
            _req(payload={"amount": 1_000, "supplier_id": "SUP-001", "category": "hardware"})
        )
        expl = self.explainer.explain(resp)
...
class TestPolicySimulator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for amt in [5_000, 10_000, 50_000, 100_000, 700_000]:
            self.pipeline.process(
                _req(payload={"amount": amt, "supplier_id": "SUP-001", "category": "hardware"})
            )
    ...
    def test_simulate_strict_policy_shows_impact(self):
        def strict(payload, ctx):
            if float(payload.get("amount", 0)) > 1_000:
                return PolicyEvaluation("STRICT", "Strict", "fail", "[STRICT] Over $1K")
            return PolicyEvaluation("STRICT", "Strict", "pass", "OK")

        policy = Policy(
            policy_id="STRICT",
            policy_name="Strict $1K",
    ...
    def test_simulation_result_to_dict(self):
        ...
        policy = Policy(
            policy_id="N", policy_name="N", decision_types=[DecisionType.PROCUREMENT], rule=noop
        )
        result = self.sim.simulate_policy(policy, lookback_hours=999)
        d = result.to_dict()
        for k in [
...
class TestMCPToolScanner(unittest.TestCase):
    ...
    def test_parameter_description_injection_detected(self):
        ...
        self.assertTrue(
            any(
                f.location.startswith("parameters.properties.instructions") for f in report.findings
            )
        )
...
class TestMCPGovernanceGateway(unittest.TestCase):
    ...
    def test_tool_call_with_executor(self):
        ...
        try:
            result = self.gateway.call_tool("web_search", {"query": "test"}, tool_fn=my_tool)
...
class TestOPARegoAdapter(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.adapter = OPARegoAdapter(opa_url=None, fallback="pass")
    ...
    def test_fail_open_on_unreachable(self):
        ...
        adapter = OPARegoAdapter(
            opa_url="http://localhost:19999", fallback="pass", timeout_s=0.1  # no server
        )
        ctx = DecisionContext()
    ...
    def test_fail_closed_on_unreachable(self):
        ...
        adapter = OPARegoAdapter(
            opa_url="http://localhost:19999", fallback="fail", timeout_s=0.1  # no server
        )
        ctx = DecisionContext()
        ev = adapter.evaluate({"amount": 1000}, ctx)
...
class TestBulkBatchAPI(unittest.TestCase):
    ...
    def test_batch_respects_max_500_limit(self):
        ...
        decisions = [
            {"agent_id": f"a{i}", "decision_type": "custom", "payload": {"x": i}}
            for i in range(500)
        ]
        ...
        decisions_small = [
            {"agent_id": f"a{i}", "decision_type": "custom", "payload": {"x": i}} for i in range(5)
        ]
