...
from glassbox.governance.execution_trace import ExecutionStep, ExecutionTrace, StageTimer
...
class TestSQLitePolicyRepository(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.record = PolicyRecord(
            policy_id="SQL-001",
            policy_name="SQL Test Policy",
            decision_types=["financial"],
            rule_type="python",
            rule_body="pass",
            version="1.0",
        )
...
class TestEventBus(unittest.TestCase):
    ...
    def test_wildcard_subscription(self):
        ...
        self.bus.publish_sync(DecisionBlocked("d1", "a1", "financial", ["VIOL"], 0.5))
    ...
    def test_pipeline_publishes_events(self):
        ...
        p.process(
            DecisionRequest(
                "ev_agent", DecisionType.PROCUREMENT, {"amount": 700000, "category": "hardware"}
            )
        )
...
class TestDeclarativeRule(unittest.TestCase):
    ...
    def test_and_logic_all_match(self):
        rule = DeclarativeRule(
            ...
            result="fail",
            message="Over limit without contract",
            logic="and",
        )
...
class TestWorkflowEngine(unittest.TestCase):
    ...
    def test_get_by_decision(self):
        wf = self.engine.create_from_decision("d-find", "agent_e", "inventory", 20.0, [])
    ...
    def test_workflow_publishes_events(self):
        received = []
        lock = threading.Lock()
        self.bus.subscribe("decision.pending_review", lambda e: received.append(e) or None)
...
class TestExecutionTrace(unittest.TestCase):
    ...
    def test_pipeline_trace_enabled(self):
        ...
        resp = p.process(
            DecisionRequest(
                "trace_agent", DecisionType.PROCUREMENT, {"amount": 700000, "category": "hardware"}
            )
        )
    ...
    def test_trace_summary_string(self):
        ...
        with StageTimer(trace, 3, "Schema") as t:
            t.outcome = "passed"
        with StageTimer(trace, 6, "Policy") as t:
            t.outcome = "blocked"
        trace.finalise()
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            p = GovernancePipeline(
                ...
                risk_evaluator=__import__(
                    "glassbox.governance.risk_evaluator", fromlist=["RiskEvaluator"]
