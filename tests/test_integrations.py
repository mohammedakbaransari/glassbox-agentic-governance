...
class TestAgentChain(unittest.TestCase):
    ...
    def setUp(self):
        self.p = _pipe()
        from glassbox.orchestration.orchestrator import AgentNode, AgentOrchestrator
    ...
    def test_saga_compensates_on_block(self):
        ...
        steps = [
            ...
            AgentNode(
                ...
                "saga_b",
                DecisionType.PROCUREMENT,
                lambda ctx: {"amount": 700000, "category": "hardware"},
            ),  # blocked
        ]
        result = self.orch.run_saga(steps)
        self.assertEqual(result.status, "aborted")
...
class TestIntegrationAdapters(unittest.TestCase):
    ...
    def test_autogen_adapter_blocks_and_returns_error_dict(self):
        ...
        result = governed["transfer_funds"](
            amount=2_000_000, destination_account="ACC-001", reference="REF-001"
        )
    ...
    def test_langchain_adapter_blocks_and_raises(self):
        from glassbox.integrations.adapters import GovernanceBlockedError, LangChainAdapter
        ...
        with self.assertRaises(GovernanceBlockedError):
            governed._run(
                '{"amount": 2000000, "destination_account": "ACC-001", "reference": "REF-1"}'
            )
    ...
    def test_langchain_adapter_async_prefers_native_async_tool(self):
        ...
        governed = adapter.wrap_tool(tool)
        result = asyncio.run(
            governed._arun('{"amount": 5000, "supplier_id": "SUP-001", "category": "hardware"}')
        )
...
class TestRAGRetrievalGovernor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.gov = RAGRetrievalGovernor(source_registry=reg, min_relevance=0.4, max_age_days=365)
        self.Chunk = RetrievedChunk

    def _chunk(self, cid, content="Normal content", source="doc://policy-manual", relevance=0.8):
        return self.Chunk(chunk_id=cid, content=content, source=source, relevance_score=relevance)

    def test_clean_chunks_pass(self):
        # Use different content to avoid deduplication
        chunks = [
            self._chunk("c1", content="Policy content section 1"),
    ...
    def test_duplicate_chunk_blocked(self):
        chunks = [
            self._chunk("c1", content="Same content"),
    ...
    def test_open_registry_allows_any_source(self):
        from glassbox.rag.governance import ApprovedSourceRegistry, RAGRetrievalGovernor
    ...
    def test_with_retriever(self):
        ...
        def mock_retriever(query):
            return [
                ...
                RetrievedChunk(
                    "c2", "High-risk categories require approval", "doc://kb", relevance_score=0.8
                ),
            ]
...
class TestMultiTenancy(unittest.TestCase):
    ...
    def test_context_isolation_validator(self):
        ...
        self.registry.get("org_a")
        self.registry.get("org_b")
        self.registry.get("org_c")
    ...
    def test_concurrent_multi_tenant_requests(self):
        ...
        threads = [
            threading.Thread(target=process_tenant, args=(f"tenant_{i}", 10)) for i in range(5)
        ]
        for t in threads:
            t.start()
        for t in threads:
...
class TestComplianceCatalogue(unittest.TestCase):
    ...
    def test_update_control_status(self):
        ok = self.cat.update_status(
            "EUAI.A9", "implemented", notes="Verified via RiskEvaluator review"
        )
        self.assertTrue(ok)
    ...
    def test_pipeline_auto_collects_evidence(self):
        ...
        p.process(
            DecisionRequest(
                "block_agent", DecisionType.PROCUREMENT, {"amount": 700000, "category": "hardware"}
            )
        )
            ...
            try:
                for j in range(10):
                    self.cat.record_evidence(
                        "AIRM.MG.02", "test", decision_id=f"d-{i}-{j}", agent_id=f"agent_{i}"
                    )
            except Exception as e:
                with lock:
                    errors.append(str(e))
...
class TestAsyncAuditWrites(unittest.TestCase):
    ...
    def test_async_writes_pipeline_param(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            p = GovernancePipeline(echo=False, log_dir=tmpdir, async_audit_writes=True)
            for _ in range(20):
                p.process(_req())
    ...
    def test_async_writes_vs_sync_produces_same_count(self):
        ...
        time.sleep(0.05)
        self.assertEqual(len(p_sync.audit_logger.get_all()), len(p_async.audit_logger.get_all()))
    ...
    def test_orchestrator_with_full_pipeline(self):
        ...
        nodes = [
            ...
            AgentNode(
                ...
                DecisionType.FINANCIAL,
                lambda ctx: {
                    "amount": ctx.get("procurement.payload", {}).get("amount", 0),
                    "destination_account": "ACC-001",
                    "reference": "PO-001",
                },
...
if __name__ == "__main__":
    ...
    classes = [
        ...
        TestRAGQueryGovernor,
        TestRAGRetrievalGovernor,
        TestAgenticRAGOrchestrator,
        TestMultiTenancy,
