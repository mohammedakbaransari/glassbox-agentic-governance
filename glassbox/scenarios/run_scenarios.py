...
def _res(label, resp, indent=2):
    ...
    if resp.audit_record and resp.audit_record.context:
        ...
        if d:
            cd = f"  chain={d}"
    print(
        f"{pad}[{icon}] {label:<42} {resp.final_status.value:<16} {risk}{cd}  "
...
""")
    ...
    for case in cases:
        ...
        _res(
            label,
            p.process(DecisionRequest("procurement_agent", DecisionType.PROCUREMENT, payload, ctx)),
        )
    s = p.stats
    print(
...
""")
    ...
    cases = [
        ...
        (
            "18% — warning zone",
            {"new_price": 118.0, "previous_price": 100.0, "product_id": "P-004"},
        ),
        (
            "32% — policy breach",
            {"new_price": 132.0, "previous_price": 100.0, "product_id": "P-005"},
        ),
        (
            "400% — corrupted signal",
            {"new_price": 499.0, "previous_price": 100.0, "product_id": "P-006"},
        ),
        (
            "Price below floor",
            {
...
""")
    ...
    eng = PolicyEngine()
    eng.disable("PROC-001")
    eng.register(
        Policy(
            policy_id="PROC-001-STRICT",
            policy_name="Strict Limit",
    ...
    for r in results:
        ...
        print(
            f"  REPLAYED  orig={r['original_status']:<14} new={r['replayed_status']:<14}{changed}"
        )
...
""")
    ...
    base_payload = {
        "amount": 420000,
        "destination_account": "ACC-FX-001",
    ...
    chains = [
        ...
        (
            "treasury → fx → compliance (depth 3)",
            ["treasury_agent", "fx_pricing_agent", "compliance_agent"],
            0.81,
        ),
        (
            "4-level chain, degraded conf (depth 4)",
            ["treasury_agent", "fx_pricing_agent", "compliance_agent", "risk_agent"],
            0.58,
        ),
    ]
...
""")
    ...
    def _agg_budget(payload, ctx, pipeline=p):
        ...
        return PolicyEvaluation(
            "AGG-001", "Fleet Budget Control", "pass", f"Fleet budget at {util:.0f}%."
        )

    p.policy_engine.register(
        Policy(
            policy_id="AGG-001",
            policy_name="Fleet Procurement Budget",
            decision_types=[DecisionType.PROCUREMENT],
            rule=_agg_budget,
    ...
    for agent, amount, cat in orders:
        ...
        if r.final_status == FinalStatus.EXECUTED:
            cumulative += amount
        note = ""
        if any("AGG-001" in v for v in r.policy_violations):
            note = "  ← FLEET BUDGET EXCEEDED"
        elif any("AGG-001" in w for w in r.policy_warnings):
            note = "  ← budget warning"
