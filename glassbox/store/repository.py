...
class PolicyRepository(ABC):
    ...
    @abstractmethod
    def list_all(self, status: Optional[str] = None) -> List[PolicyRecord]: ...
...
class SQLiteAuditRepository(AuditRepository):
    ...
    def save(self, record: AuditRecord, tenant_id: Optional[str] = None) -> None:
        ...
        if (
            ...
            and record.context
            and isinstance(record.context.metadata, dict)
        ):
            ...
            with self._conn() as conn:
                ...
                """,
                    (
                        ...
                        int(
                            record.circuit_breaker_result.triggered
                            if record.circuit_breaker_result
                            else False
                        ),
                        effective_tenant,
    ...
    def query(
        ...
        with self._lock:
            with self._conn() as conn:
                ...
                return [
                    self._deserialize_audit_record(json.loads(r["full_record_json"])) for r in rows
                ]
    ...
    def aggregate_spend(
        ...
        with self._lock:
            with self._conn() as conn:
                row = conn.execute(
                    # `where` is hardcoded literals only; variable content is bound via `?`.
                    f"SELECT COALESCE(SUM(payload_amount),0) FROM audit_records WHERE {where}",  # nosec B608
                    params,
                ).fetchone()
                return float(row[0])
