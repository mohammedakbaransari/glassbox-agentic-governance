...
class Policy:
    ...
    def __init__(
        ...
        decision_types: Optional[List] = None,
        ...
        **kwargs,
        ...
        self.name: str = name if name is not None else self.id
...
class ReadOnlySnapshot:
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        try:
            ...
            raise AttributeError(f"Field '{name}' not found in snapshot") from exc
...
class SnapshotPattern:
    ...
    def readonly_view(
        ...
        frozen = set(fields) if fields is not None else set(data.keys())
...
class PolicyEngine:
    ...
    def __init__(
        ...
        """
        self._policies: Dict[str, Policy] = {}  # policy_id -> Policy object
        self._policies_by_type: Dict[Any, Tuple[Policy, ...]] = {}  # DecisionType -> Policies
    ...
    def register(self, policy: Policy, warn_conflicts: bool = True) -> None:
        ...
        with self._lock:
            ...
            if policy.enabled:
                for dt in policy.decision_types:
                    updated_policies: List[Policy] = list(self._policies_by_type.get(dt, ()))
                    updated_policies.append(policy)
                    self._policies_by_type[dt] = tuple(updated_policies)
...
def _get_sanctioned_countries() -> frozenset:
    ...
    if raw is not None:
        return (
            frozenset(str(c).upper() for c in raw)
            if isinstance(raw, (list, tuple, set))
            else _DEFAULT_SANCTIONED_COUNTRIES
        )
...
def _get_debarred_suppliers() -> frozenset:
    ...
    if raw is not None:
        return (
            frozenset(str(s).upper() for s in raw)
            if isinstance(raw, (list, tuple, set))
            else _DEFAULT_DEBARRED_SUPPLIERS
        )
...
def _it_ops_policy_maintenance_window(payload: Dict, ctx: DecisionContext) -> PolicyEvaluation:
    ...
    is_during_maintenance = payload.get("during_maintenance_window", False) or payload.get(
...
def _it_ops_policy_change_log(payload: Dict, ctx: DecisionContext) -> PolicyEvaluation:
    ...
    action = (
        str(payload.get("action", "") or "").lower().strip().replace(" ", "_").replace("-", "_")
    )
...
DEFAULT_POLICIES: List[Policy] = [
    ...
    Policy(
        ...
        rule=_procurement_policy_amount_limit,
    ),
    Policy(
        policy_id="PROC-002",
        policy_name="Supplier Known Check",
        decision_types=[DecisionType.PROCUREMENT],
        rule=_procurement_policy_supplier_known,
    ),
    Policy(
        policy_id="PROC-003",
        policy_name="Category Risk Check",
        decision_types=[DecisionType.PROCUREMENT],
        rule=_procurement_policy_category_risk,
    ),
    Policy(
        policy_id="PROC-004",
        policy_name="Sole Source Justification",
        decision_types=[DecisionType.PROCUREMENT],
        rule=_procurement_policy_sole_source,
    ),
    Policy(
        policy_id="PROC-005",
        policy_name="Audit Trail Requirement",
        decision_types=[DecisionType.PROCUREMENT],
        rule=_procurement_policy_audit_trail,
    ),
    Policy(
        policy_id="PROC-006",
        policy_name="Sanctions & Debarment Check",
        decision_types=[DecisionType.PROCUREMENT],
        rule=_procurement_policy_sanctions,
    ),
    # ── Pricing ──────────────────────────────────────────────────────────────
    Policy(
        policy_id="PRICE-001",
        policy_name="Price Change Limit",
        decision_types=[DecisionType.PRICING],
        rule=_pricing_policy_change_limit,
    ),
    Policy(
        policy_id="PRICE-002",
        policy_name="Floor Price Limit",
        decision_types=[DecisionType.PRICING],
        rule=_pricing_policy_floor_limit,
    ),
    Policy(
        policy_id="PRICE-003",
        policy_name="Ceiling Price Limit",
        decision_types=[DecisionType.PRICING],
        rule=_pricing_policy_ceiling_limit,
    ),
    Policy(
        policy_id="PRICE-004",
        policy_name="Competitor Price Benchmark",
        decision_types=[DecisionType.PRICING],
        rule=_pricing_policy_competitor_check,
    ),
    # ── Financial ────────────────────────────────────────────────────────────
    Policy(
        policy_id="FIN-001",
        policy_name="Transfer Limit",
        decision_types=[DecisionType.FINANCIAL],
        rule=_financial_policy_transfer_limit,
    ),
    Policy(
        policy_id="FIN-002",
        policy_name="Wire Transfer Velocity",
        decision_types=[DecisionType.FINANCIAL],
        rule=_financial_policy_wire_velocity,
    ),
    Policy(
        policy_id="FIN-003",
        policy_name="Missing Counterparty",
        decision_types=[DecisionType.FINANCIAL],
        rule=_financial_policy_counterparty_check,
    ),
    Policy(
        policy_id="FIN-004",
        policy_name="CTR Cash Threshold",
        decision_types=[DecisionType.FINANCIAL],
        rule=_financial_policy_ctr_threshold,
    ),
    Policy(
        policy_id="FIN-005",
        policy_name="Structuring Detection",
        decision_types=[DecisionType.FINANCIAL],
        rule=_financial_policy_structuring,
    ),
    # ── Clinical ─────────────────────────────────────────────────────────────
    Policy(
        policy_id="CLIN-001",
        ...
        rule=_clinical_policy_controlled_substance,
    ),
    Policy(
        policy_id="CLIN-002",
        policy_name="Dosage Safety Check",
        decision_types=[DecisionType.CLINICAL],
        rule=_clinical_policy_dosage_check,
    ),
    # ── Trading ──────────────────────────────────────────────────────────────
    Policy(
        policy_id="TRADE-001",
        policy_name="Position Limit",
        decision_types=[DecisionType.TRADING],
        rule=_trading_policy_position_limit,
    ),
    Policy(
        policy_id="TRADE-002",
        policy_name="Fat Finger Check",
        decision_types=[DecisionType.TRADING],
        rule=_trading_policy_fat_finger,
    ),
    # ── IT Operations ────────────────────────────────────────────────────────
    Policy(
        policy_id="IT-OPS-002",
        policy_name="Maintenance Window Enforcement",
        decision_types=[DecisionType.IT_OPS],
        rule=_it_ops_policy_maintenance_window,
    ),
    Policy(
        policy_id="IT-OPS-003",
        policy_name="Service Criticality Gate",
        decision_types=[DecisionType.IT_OPS],
        rule=_it_ops_policy_service_criticality,
    ),
    Policy(
        policy_id="IT-OPS-004",
        policy_name="Change Log Requirement",
        decision_types=[DecisionType.IT_OPS],
        rule=_it_ops_policy_change_log,
    ),
    # ── Human Resources ──────────────────────────────────────────────────────
    Policy(
        policy_id="HR-001",
        policy_name="Salary Change Limit",
        decision_types=[DecisionType.HR],
        rule=_hr_policy_salary_limits,
    ),
    Policy(
        policy_id="HR-002",
        policy_name="Promotion Approval Gate",
        decision_types=[DecisionType.HR],
        rule=_hr_policy_promotion_restrictions,
    ),
    Policy(
        policy_id="HR-003",
        policy_name="Access Provisioning Security",
        decision_types=[DecisionType.HR],
        rule=_hr_policy_access_provisioning,
    ),
    # ── Compliance ───────────────────────────────────────────────────────────
    Policy(
        policy_id="COMPLIANCE-001",
        ...
        rule=_compliance_policy_pii_exposure,
    ),
    Policy(
        policy_id="COMPLIANCE-002",
        policy_name="Data Residency Compliance",
        decision_types=[DecisionType.CUSTOM],
        rule=_compliance_policy_data_residency,
    ),
    Policy(
        policy_id="COMPLIANCE-003",
        policy_name="Breach Notification Requirement",
        decision_types=[DecisionType.CUSTOM],
        rule=_compliance_policy_breach_notification,
    ),
    # ── General / Content ────────────────────────────────────────────────────
    Policy(
        policy_id="GEN-001",
        policy_name="PII Exposure Prevention",
        decision_types=[DecisionType.CONTENT],
        rule=_policy_pii_prevention,
    ),
    Policy(
        policy_id="GEN-002",
        policy_name="EU Automated Decision Gate",
        decision_types=[DecisionType.CONTENT],
        rule=_policy_eu_automated_decision,
    ),
    # ── Legal ─────────────────────────────────────────────────────────────────
    Policy(
        policy_id="LEGAL-001",
        policy_name="Contract Authority Limit",
        decision_types=[DecisionType.LEGAL],
        rule=_legal_policy_contract_authority,
    ),
    Policy(
        policy_id="LEGAL-002",
        policy_name="Legal Hold Compliance",
        decision_types=[DecisionType.LEGAL],
        rule=_legal_policy_hold_compliance,
    ),
    # ── Security / Governance ─────────────────────────────────────────────────
...
def _fleet_budget_rule(payload: Dict, ctx: "DecisionContext") -> "PolicyEvaluation":
    ...
    spend = float(
        payload.get("fleet_spend", payload.get("amount", payload.get("total_cost", 0))) or 0
    )
