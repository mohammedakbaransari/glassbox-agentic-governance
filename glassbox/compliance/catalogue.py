...
CONTROL_CATALOGUE: List[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "framework": "OWASP Agentic Top 10",
        "category": "A04 Resource Consumption",
    ...
    },
    {
        ...
        "framework": "OWASP Agentic Top 10",
        "category": "A07 Cascading Failures",
    ...
    },
    {
        ...
        "framework": "OWASP Agentic Top 10",
        "category": "A10 Multi-Agent Trust",
    ...
    # ── SOCI Act 2018 ─────────────────────────────────────────────────────────
    {
        "control_id": "SOCI.S30BC",
        "framework": "SOCI Act 2018",
        "category": "Positive Security Obligation",
    ...
    {
        "control_id": "CSA24.INCIDENT",
        "framework": "Cyber Security Act 2024",
    ...
    # ── Purdue Model 2.0 ─────────────────────────────────────────────────────
    {
        "control_id": "PURDUE.L3-L4",
        "framework": "Purdue Model 2.0",
        "category": "Zone Separation",
...
class ComplianceCatalogue:
    ...
    def _init_schema(self) -> None:
        with self._lock:
            with self._conn() as conn:
                ...
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_ctrl_status ON controls(implementation_status)"
                )
    ...
    def list_controls(
        ...
        if status:
            where.append("implementation_status=?")
        ...
        with self._lock:
            with self._conn() as conn:
                ...
                for r in rows:
                    ...
                    coverage = round(
                        (r["implemented"] + r["partial"] * 0.5) / max(applicable, 1) * 100, 1
                    )
                    summary[r["framework"]] = {
                        "total": total,
    ...
    def gap_analysis(self, framework: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        with self._lock:
            with self._conn() as conn:
                rows = conn.execute(
                    # `where` is hardcoded literals only; `framework` is bound via `?`.
                    f"SELECT * FROM controls WHERE {where} ORDER BY framework, control_id",  # nosec B608
                    params,
                ).fetchall()
                return [dict(r) for r in rows]
